...
DWORD WINAPI CClient::_WorkerThread(LPVOID lpParam)
{
    ...
    TCHAR szTemp[MAX_BUFFER_LEN] = {0};
    TCHAR szTempRecv[MAX_BUFFER_LEN] = {0};
...
//         }
        ...
        _sntprintf(szTemp, MAX_BUFFER_LEN, _T("第%d条信息：%s"), i, pParams->szBuffer);
        nBytesSent = send(pParams->sock, (char*)szTemp, _tcslen(szTemp)*sizeof(TCHAR)/sizeof(char), 0);
        if (SOCKET_ERROR == nBytesSent)
        {
            TRACE(_T("错误：发送 第%d次信息失败，错误代码：%ld\n"), i, WSAGetLastError());
            return 1;
        }
        //TRACE(_T("发送成功: %s\n"), szTemp);
        //pClient->ShowMessage(_T("发送成功: %s"), szTemp);

        memset(szTempRecv, 0, sizeof(szTempRecv));
        nBytesRecv = recv(pParams->sock, (char*)szTempRecv, sizeof(szTempRecv), 0);
        if (SOCKET_ERROR == nBytesSent)
        {
            TRACE(_T("错误：接收 第%d次信息失败，错误代码：%ld\n"), i, WSAGetLastError());
            return 1;
        }
        //pClient->ShowMessage(_T("接收成功: %s"), szTempRecv);
...
bool  CClient::EstablishConnections()
{
    ...
    for (int i = 0; i < m_nThreads; i++)
    {
        ...
        _sntprintf(m_pParamsWorker[i].szBuffer, MAX_BUFFER_LEN, _T("%d号线程 发送数据 %s"), i + 1, m_strMessage.GetString());
...
bool CClient::ConnetToServer(SOCKET *pSocket, CString strServer, int nPort)
{
    ...
    *pSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    // 生成地址信息
    Server = gethostbyname(CT2A(strServer.GetString()));
    if (Server == NULL)
    {
        closesocket(*pSocket);
        TRACE("错误：无效的服务器地址.\n");
