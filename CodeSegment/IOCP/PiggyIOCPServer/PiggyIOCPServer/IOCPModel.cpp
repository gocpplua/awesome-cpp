...
DWORD WINAPI CIOCPModel::_WorkerThread(LPVOID lpParam)
{
    ...
    while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
    {
        ...
        else
        {
            ...
            {
                char* addr = inet_ntoa(pSocketContext->m_ClientAddr.sin_addr);
                CString sAddr = CA2T(addr);
                pIOCPModel->_ShowMessage(_T("客户端 %s:%d 断开连接."), sAddr, ntohs(pSocketContext->m_ClientAddr.sin_port));
...
bool CIOCPModel::_InitializeListenSocket()
{
    ...
    ServerAddress.sin_addr.s_addr = inet_addr(CT2A(m_strIP.GetString()));
...
bool CIOCPModel::_DoAccpet(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext)
{
    ...
    //     CString sAddr = CA2T(addr);
    //     CString sBuf = (TCHAR*)(pIoContext->m_wsaBuf.buf);
    // 
    //     this->_ShowMessage(_T("客户端 %s:%d 连入."), sAddr, ntohs(ClientAddr->sin_port));
    //     this->_ShowMessage(_T("客户端 %s:%d 信息：%s"), sAddr, ntohs(ClientAddr->sin_port), sBuf);
...
bool CIOCPModel::_PostSend(PER_IO_CONTEXT* pIoContext)
{
    ...
    INT nResultCode = WSASend(pIoContext->m_sockAccept, &pIoContext->m_wsaBuf, 1, &dwThancferred, 0, &pIoContext->m_Overlapped, NULL);
...
//     CString sAddr = CA2T(addr);
//     CString sBuf = (TCHAR*)(pIoContext->m_wsaBuf.buf);
//     this->_ShowMessage(_T("收到  %s:%d 信息：%s"), sAddr, ntohs(ClientAddr->sin_port), sBuf);

    // 然后开始投递下一个WSASend请求 将数据包发回去
